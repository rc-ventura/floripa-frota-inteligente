...
|---|---|---|---|
| Abastecimento | CSV depositado em pasta monitorada | Exportação de planilha paralela | Placa com e sem hífen; datas `dd/mm/aaaa` e `aaaa-mm-dd`; litros com vírgula decimal |
...
**Regra de reconciliação:** a **placa do veículo** é a chave que une todas as fontes. O pipeline normaliza toda placa para o formato canônico `AAA9999` (maiúsculas, sem hífen) antes de qualquer cruzamento.
...
2. **Validate & Transform** — regras aplicadas sobre o staging:
   - Normalização de placa para o formato canônico (regex + upper);
...
erDiagram
    ...
    VEICULO {
        string placa PK "formato canonico AAA9999"
...
| D7 | Identificador único | **Placa normalizada** como chave natural | ID sintético global, chassi/RENAVAM | Presente em todas as fontes legadas; resolve o risco de ausência de identificador comum; chassi/RENAVAM documentados como chave secundária de desempate para produção |
